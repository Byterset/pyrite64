...
fm_vec3_t Collider::support(const fm_vec3_t &dir) const {
  switch(type) {
    ...
    case ShapeType::Sweep:    return sweep.support(dir);
  }
...
AABB Collider::boundingBox(const fm_quat_t *rotation) const {
  switch(type) {
    ...
    case ShapeType::Sweep:    return sweep.boundingBox(rotation);
  }
  __builtin_unreachable();
}
...
fm_vec3_t Collider::inertiaTensor(float mass) const {
  switch(type) {
    ...
    case ShapeType::Sweep:    return sweep.inertiaTensor(mass);
  }
  __builtin_unreachable();
}
