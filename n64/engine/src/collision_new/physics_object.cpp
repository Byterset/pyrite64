 ...
 * @license MIT
 */
#include "collision_new/physics_object.h"
#include <cassert>
#include <cmath>
...
namespace P64::CollNew {
  ...
  void PhysicsObject::init(EntityId id, Collider *coll, uint16_t layers,
    ...
    timeScalar = 1.0f;
    gravityScalar = 1.0f;
    angularDamping = 0.03f * (60.0f / 30.0f);
  ...
  void PhysicsObject::integrateVelocity() {
    if(isTrigger || isKinematic || isSleeping) return;

    float dt = FIXED_DT * timeScalar;

    // Apply gravity
    if(hasGravity) {
      velocity.y += GRAVITY_CONSTANT * gravityScalar * dt;
    }
    ...
    if(speedSq > TERMINAL_SPEED * TERMINAL_SPEED) {
      velocity = vec3Scale(velocity, TERMINAL_SPEED / sqrtf(speedSq));
    }
  ...
  void PhysicsObject::integrateAngularVelocity() {
    if(isTrigger || isKinematic || isSleeping) return;
    ...
    if(!vec3IsZero(torqueAccumulator)) {
      ...
      angularVelocity = vec3Add(angularVelocity, vec3Scale(angAccel, dt));
      torqueAccumulator = vec3Zero();
    }
    ...
    if(angSpeedSq > TERMINAL_ANGULAR_SPEED_SQ) {
      angularVelocity = vec3Scale(angularVelocity, TERMINAL_ANGULAR_SPEED / sqrtf(angSpeedSq));
    ...
    angularVelocity = vec3Scale(angularVelocity, 1.0f - dampFactor * dt);
  }

  void PhysicsObject::integratePosition() {
    if(isTrigger || isKinematic || isSleeping) return;
    if(!position) return;
  ...
  void PhysicsObject::integrateRotation() {
    if(isTrigger || isKinematic || isSleeping) return;
    if(!rotation) return;
  ...
  void PhysicsObject::accelerate(const fm_vec3_t &accel) {
    acceleration = vec3Add(acceleration, accel);
    if(isSleeping) wake();
  }

  void PhysicsObject::setVelocity(const fm_vec3_t &vel) {
    velocity = vel;
    if(isSleeping) wake();
  }

  void PhysicsObject::applyLinearImpulse(const fm_vec3_t &impulse) {
    if(isKinematic || isTrigger) return;
    velocity = vec3Add(velocity, vec3Scale(impulse, invMass));
    if(isSleeping) wake();
  }

  void PhysicsObject::applyTorque(const fm_vec3_t &torque) {
    if(isKinematic || isTrigger) return;
    torqueAccumulator = vec3Add(torqueAccumulator, torque);
    if(isSleeping) wake();
  }

  void PhysicsObject::applyAngularImpulse(const fm_vec3_t &angImpulse) {
    if(isKinematic || isTrigger) return;
    angularVelocity = vec3Add(angularVelocity, applyWorldInertia(angImpulse));
    if(isSleeping) wake();
  }

  void PhysicsObject::setAngularVelocity(const fm_vec3_t &angVel) {
    angularVelocity = angVel;
    if(isSleeping) wake();
  }

  void PhysicsObject::applyForceAtPoint(const fm_vec3_t &force, const fm_vec3_t &worldPoint) {
    if(isKinematic || isTrigger) return;
  ...
  void PhysicsObject::recalculateAABB() {
  ...
  void PhysicsObject::updateWorldInertia() {
    if(!rotation) {
  ...
  void PhysicsObject::applyPositionConstraints() {
    if(!position) return;
  ...
  void PhysicsObject::gjkSupport(const fm_vec3_t &direction, fm_vec3_t &output) const {
    if(!collider) {
  ...
  Contact *PhysicsObject::nearestContact() const {
  ...
  bool PhysicsObject::isTouching(EntityId id) const {
    for(Contact *c = activeContacts; c; c = c->next) {
      if(c->otherObject && c->otherObject->entityId == id) return true;
    }
    return false;
  }

  void physicsObjectGjkSupport(const void *data, const fm_vec3_t &direction, fm_vec3_t &output) {
    auto *obj = static_cast<const PhysicsObject *>(data);
