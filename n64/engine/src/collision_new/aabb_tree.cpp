...
void AABBTree::init(int capacity) {
  ...
  nodes_ = static_cast<AABBTreeNode *>(calloc(nodeCapacity_, sizeof(AABBTreeNode)));
  assert(nodes_);
...
NodeProxy AABBTree::allocateNode() {
  ...
  if(freeList_ == NULL_NODE) {
    ...
    nodes_ = static_cast<AABBTreeNode *>(realloc(nodes_, nodeCapacity_ * sizeof(AABBTreeNode)));
    assert(nodes_);

    // Zero-init the new portion and chain it into the free list.
    memset(&nodes_[oldCap], 0, (nodeCapacity_ - oldCap) * sizeof(AABBTreeNode));
